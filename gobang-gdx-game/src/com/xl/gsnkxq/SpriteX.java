...
public class SpriteX {
  ...
  public static final int TRANS_MIRROR = 2; //横向翻转
  ...
  //collision type
  public static final int COLLISION_INTERSECT = 1; //空心矩形检测
  public static final int COLLISION_INCLUSION = 2; //矩形内部检测
  ...
  public SpriteX(String spxName, Texture image) {
    loadSpx(spxName, image);
  }
  
  public SpriteX(InputStream in,Texture image)
  {
	  loadSpx(in,image);
  }

  public SpriteX(String spxName, Texture[] tiles) {
    loadSpx(spxName, image);
    this.tiles = tiles;
    this.tileMode = true;
  }
...
//          loadSpx(spxName, spximg);
  ...
  public SpriteX(SpriteX spx) {
	...
	public void loadSpx(InputStream is, Texture image) {
		try {
...
FileLog.e("setImage");
			...
			ex.printStackTrace();
			FileLog.e(ex.toString());
		}
	}
  public void loadSpx(String spxName, Texture image)
  {
    
     // InputStream is = spxName.getClass().getResourceAsStream(spxName);
	  FileHandle file = Gdx.files.internal(spxName);
    ...
    for (int i = 0; i < tileCount; i++) {
      if (tileUse[i] == 1) {
        if (tiles[i] == null) {
          ...
          Texture spximg = null;
          try {
            spximg = new Texture(fullName);
			FileLog.e("createImage "+fullName);
          }
          catch (Exception ex) {
            System.out.println("can't load spx image\n");
          }
...
//    }
  ...
  public float getReferencePointX(int index) {
  ...
  public float getReferencePointY(int index) {
    int frameIndex = getSequenceFrame();
    int frameTileCount = frameData[frameIndex][FRAME_TILE_COUNT_BIT];
    int frameCollisionCount = frameData[frameIndex][
        FRAME_COLLISION_COUNT_BIT];
  ...
  float getTransformedReferenceX(float x, float y, int transform) {
    float t_x = 0;
    switch (transform) {
      case TRANS_NONE:
        t_x = x;
...
//        break;
  ...
  public float getCollidesX(int frame, int index) {
  ...
  public float getCollidesY(int frame, int index) {
    if (index < 0 || index >= frameData[frame][FRAME_COLLISION_COUNT_BIT]) {
      throw new ArrayIndexOutOfBoundsException();
    }
    else {
  ...
  public float getCollidesX(int index) {
    int frameIndex = getSequenceFrame();
    return getCollidesX(frameIndex, index);
  }

  public float getCollidesY(int index) {
    int frameIndex = getSequenceFrame();
  ...
  //***************************************************************
	public float getCollidesX2(int frame, int index) {
		if (index < 0 || index >= frameData[frame][FRAME_COLLISION_COUNT_BIT]) {
			throw new ArrayIndexOutOfBoundsException();
		}
		else {
	...
	public float getCollidesY2(int frame, int index) {
		if (index < 0 || index >= frameData[frame][FRAME_COLLISION_COUNT_BIT]) {
			throw new ArrayIndexOutOfBoundsException();
		}
		else {
	...
	public int getCollidesWidth2(int frame, int index) {
		if (index < 0 || index >= frameData[frame][FRAME_COLLISION_COUNT_BIT]) {
			throw new ArrayIndexOutOfBoundsException();
		}
		else {
	...
	public int getCollidesHeight2(int frame, int index) {
		if (index < 0 || index >= frameData[frame][FRAME_COLLISION_COUNT_BIT]) {
			throw new ArrayIndexOutOfBoundsException();
		}
		else {
	...
	public float getCollidesX2(int index) {
		int frameIndex = getSequenceFrame();
		return getCollidesX(frameIndex, index);
	}

	public float getCollidesY2(int index) {
		int frameIndex = getSequenceFrame();
		return getCollidesY2(frameIndex, index);
	}

	public int getCollidesWidth2(int index) {
		int frameIndex = getSequenceFrame();
		return getCollidesWidth(frameIndex, index);
	}

	public int getCollidesHeight2(int index) {
		int frameIndex = getSequenceFrame();
		return getCollidesHeight(frameIndex, index);
	}
  ...
  public boolean collidesWith(SpriteX spx, int spxCollides, int thisCollides,
    ...
    float x = spx.getCollidesX(spxCollides);
    float y = spx.getCollidesY(spxCollides);
    int width = spx.getCollidesWidth(spxCollides);
    int height = spx.getCollidesHeight(spxCollides);

    return collidesWith((int)x, (int)y, width, height, thisCollides, type);
  }
	public boolean collidesWith2(SpriteX spx, int spxCollides, int thisCollides,
								int type) {
		if (!spx.visible || !this.visible) {
			return false;
		}

		int x = (int)spx.getCollidesX(spxCollides);
		int y = (int)spx.getCollidesY2(spxCollides);
		int width = spx.getCollidesWidth(spxCollides);
		int height = spx.getCollidesHeight(spxCollides);

		return collidesWith2(x, y, width, height, thisCollides, type);
	}
  public boolean collidesWith(Texture image, int x, int y, int collides,
                              int type) {
  ...
  public boolean collidesWith(int x, int y, int width, int height,
    ...
    int x1 = (int)this.getCollidesX(collides);
    int y1 = (int)this.getCollidesY(collides);
	...
	public boolean collidesWith2(int x, int y, int width, int height,
								int collides,//碰撞边缘间距
								int type) {
		...
		int x1 = (int)this.getCollidesX(collides);
		int y1 = (int)this.getCollidesY2(collides);
		int w1 = this.getCollidesWidth(collides);
		int h1 = this.getCollidesHeight(collides);
	   FileLog.e("coll "+x+" "+y+" "+width+" "+height+" "+x1+" "+y1+" "+w1+" "+h1+" "+collides+" "+type);
		if (type == COLLISION_INTERSECT) {
			return intersectRect2(x1, y1, w1, h1, x, y, width, height);
		}
		else if (type == COLLISION_INCLUSION) {
			return inclusionRect2(x1, y1, w1, h1, x, y, width, height);
		}
		else {
			throw new ArithmeticException();
		}
  ...
  public boolean collidesWith(SpriteX spx) {
	...
	public boolean collidesWith2(SpriteX spx) {
		if (!spx.visible || !this.visible) {
			return false;
		}
		if (this.getCollidesCount() == 0 || spx.getCollidesCount() == 0) {
		...
		return collidesWith2(spx, 0, 0, COLLISION_INTERSECT);
	}
  public boolean collidesWith(Texture image, int x, int y) {
    if (!this.visible) {
      return false;
    }
  ...
  public boolean collidesWith(int x, int y, int width, int height) {
	...
	public boolean collidesWith2(int x, int y, int width, int height) {
		...
		return collidesWith2(x, y, width, height, 0, COLLISION_INTERSECT);
	}
  
//************************************************************************
//判断空心矩形碰撞(笛卡尔)
  public static boolean intersectRect2(int x1, int y1, int width1, int height1,
                                      int x2, int y2, int width2, int height2) {

    if (y2 < y1-height1 || y2-height2 > y1 || x2 + width2 < x1 ||
        x2 > x1 + width1) {
      return false;
    }
    else {
...
//碰撞检测
//判断矩形2是否在矩形1里面(笛卡尔)
  public static boolean inclusionRect2(int x1, int y1, int width1, int height1,
                                      int x2, int y2, int width2, int height2) {
    if (y2 >= y1 &&
  ...
  public int getFrameTopPos() {
    return frameData[getSequenceFrame()][FRAME_TOP_POS_BIT] + (int)y;
  }

  public int getFrameBottomPos() {
    return frameData[getSequenceFrame()][FRAME_BOTTOM_POS_BIT] + (int)y;
  }

  public int getFrameLeftPos() {
    return frameData[getSequenceFrame()][FRAME_LEFT_POS_BIT] + (int)x;
  }

  public int getFrameRightPos() {
    return frameData[getSequenceFrame()][FRAME_RIGHT_POS_BIT] + (int)x;
  }
    ...
    if (visible) {
      ...
      if (actionTransform == this.TRANS_NONE) {
        ...
        for (int i = 0; i < tileCount; i++) {
          ...
          //XL  dy = frameData[frameIndex][2 + offset] + y;
			  dy = y-frameData[frameIndex][2 + offset];
...
//    }
  ...
  public static void drawRegionMIDP1(Graphics g, Texture src, int x_src,
                                     int y_src,
  ...
  public static void drawRegionNokia(Graphics g, Texture src, int x_src,
                                     int y_src,
                                     int width, int height, int transform,
                                     int x_dest,
                                     int y_dest, int anchor) {
 ...
 //   dg.drawImage(src, x_dest - x_offset, y_dest - y_offset, 0,  manipulation);
  ...
  public static void drawRegion(Graphics g, Texture src, int x_src, int y_src,
                                int width, int height, int transform,
                                int x_dest,
                                int y_dest, int anchor) {
    //Gdx.app.log("drawRegin",""+transform);
//    //MIDP2.0
